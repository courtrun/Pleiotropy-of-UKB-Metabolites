...
all.res <- data.table::rbindlist(lapply(mrfiles, function(x) {load(sprintf("mrbase_local/%s.RData", x)); if (!is.null(heterogeneity) && !is.na(heterogeneity) && nrow(heterogeneity) > 0) {heterogeneity$outcome = x; heterogeneity}else{NULL}})) %>% 
...
all.res %>%
    write.table("all.restructured.heterogeneity.txt", quote=F, sep="\t", row.names=F, col.names=T)
