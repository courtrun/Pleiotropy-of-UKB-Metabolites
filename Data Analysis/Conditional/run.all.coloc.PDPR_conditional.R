...
library(coloc)
sdp %>% filter(condition == "othertwo", !is.na(beta)) %>% mutate(varbeta = sebeta**2) %>% select(-condition) -> varp
...
pdf("all.coloc.PDPR_conditional.pdf", useDingbats=F)
plt <- ggplot(all.coloc, aes(x=t1, y=t2, fill=PP.H4.abf)) + geom_tile() + theme_bw() + scale_fill_viridis_b()
print(plt);
dev.off()

write.table(all.coloc, "all_coloc_PDPR_conditional.tsv", quote=F, sep="\t", row.names=F, col.names=T)
