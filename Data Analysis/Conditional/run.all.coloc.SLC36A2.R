...
# no clinical measures for SLC36A2
allp <- varp
...
pdf("all.coloc.SLC36A2.pdf", useDingbats=F)
plt <- ggplot(all.coloc, aes(x=t1, y=t2, fill=PP.H4.abf)) + geom_tile() + theme_bw() + scale_fill_viridis_b()
print(plt);
dev.off()

write.table(all.coloc, "all_coloc_SLC36A2.tsv", quote=F, sep="\t", row.names=F, col.names=T)
